/* A pure C/C++ version of a Gauss-Siedel Laplacian solver to test the  speed of a C program versus that of doing it with 
...
struct Grid
{
    ...
    void setBCFunc(Real (*f)(const Real, const Real)); /*Real computeError();*/
};
...
#pragma omp parallel
    ...
    t_end = omp_get_wtime();
    std::cout << result << ";" << nx << ";" << t_end - t_start << ";" << getenv("OMP_NUM_THREADS") << std::endl;
